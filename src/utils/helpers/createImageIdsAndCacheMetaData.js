...
export default async function createImageIdsAndCacheMetaData({
  ...
  console.log(dicomUID)
  const studySearchOptions = {
    studyInstanceUID: dicomUID.studyInstanceUID,
    seriesInstanceUID: dicomUID.seriesInstanceUID,
  };

  const client = new api.DICOMwebClient({ url: wadoRsRoot });
  const instances = await client.retrieveSeriesMetadata(studySearchOptions);
  // const instances = []
  // const itkImage = await loadDICOMToITK(SeriesInstanceUID, StudyInstanceUID, wadoRsRoot)


  // await readFileMeta(instanceList)
  // console.log(instances)

  const modality = instances[0][MODALITY].Value[0];

  let imageIds = instances.map((instanceMetaData, index) => {
    ...
    //   SOPInstanceUID +
    //   '/frames/1';
    const imageId = prefix + instanceList[index].framePath
