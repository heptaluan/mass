...
class AddMarkNoduleTool extends AnnotationTool {
  constructor(
    toolProps = {},
    defaultToolProps = {
    ...
    this.cancel = (element) => {
      if (this.isDrawing) {
        ...
        this._deactivateModify(element);
        resetElementCursor(element);
        const { annotation, viewportIdsToRender, newAnnotation } =
          this.editData;
        const { data } = annotation;
    ...
    this._getTextLines = (data, targetId, isPreScaled) => {
      const cachedVolumeStats = data.cachedStats[targetId];
      const { w, h, area, mean, max, stdDev, Modality, areaUnit } =
        cachedVolumeStats;
      if (mean === undefined) {
        return;
      }
      ...
      textLines.push(`W: ${w.toFixed(2)} mm`);
      textLines.push(`H: ${h.toFixed(2)} mm`);
      return textLines;
    };
      ...
      for (let i = 0; i < targetIds.length; i++) {
        ...
        if (this._isInsideVolume(worldPos1Index, worldPos2Index, dimensions)) {
          ...
          cachedStats[targetId] = {
            ...
            w: Math.abs(worldHeight),
            h: Math.abs(worldWidth),
          };
        } else {
...
AddMarkNoduleTool.toolName = "AddMarkNodule";

export default AddMarkNoduleTool;
